        ...
        if term_1 or x_in_shadow:
            ...
            if edge_e not in for_topic_sample_edge_e:
                ...
                if x_in_shadow:
                    ...
                    print("inputto:", int(10*sim_) )
                    for_topic_sample_edge_w.append(int(10*sim_) )##test for now
                else:
                    print("inputto_:", int(10*sim_) )
                    for_topic_sample_edge_w.append(int(10*sim_) )
                if x == x.parent.left:
...
def TopicSample(T, x, max = None, min = None, sample_location = None, Begin = False): 
    ...
    def KN_to_Num(kn):
        return for_topic_sample_node_list.index(int(kn) ) 
    def Num_to_KN(N):
        return for_topic_sample_edge_t[N]
    test_ = len(for_topic_sample_edge_e) 
    Input_to_MF = (c_int*(3*test_) )()
    print("test:", len(for_topic_sample_edge_e))
    #test = KN_to_Num(for_topic_sample_edge_f[0])####
    
   # Input_to_MF_f = list(map(lambda x: for_topic_sample_node_list.index(int(x) ), for_topic_sample_edge_f) )
    #Input_to_MF_t = list(map(lambda x: for_topic_sample_node_list.index(int(x) ), for_topic_sample_edge_t) )
    #Input_to_MF_w = list(map(lambda x:x, for_topic_sample_edge_w) )##Just for the compiler not to show TypeError: only integer tensors of a single element can be converted to an index
    ##From my tests I thought it might because if the for_topic_sample_edge_w only comtain one element it whould 
    ##be concided a tensor insdead of a list of integer :)
    try:
        print("Input_to_MF_f:", Input_to_MF_f)
        print("Input_to_MF_t:", Input_to_MF_t)
        len(Input_to_MF_f)
        if Input_to_MF_f != None and Input_to_MF_f[0] > 10:
            print("no")
    except:
        pass
    
    for i in range(0, len(for_topic_sample_edge_p) - 1 ):
        Input_to_MF.append(for_topic_sample_node_list.index(int(for_topic_sample_edge_f[i]) )  ) 
        Input_to_MF.append(for_topic_sample_node_list.index(int(for_topic_sample_edge_t[i]) )  ) 
    ...
    Return_ = (ctypes.c_int*128)()
    Num_of_e = len(for_topic_sample_edge_e)
    if len(for_topic_sample_edge_p):
        print("Input_to_MF:", Input_to_MF[0])
    adder.HLPP(Num_of_nodes, Num_of_e, 0, Num_of_t, Input_to_MF , False, Return_)
